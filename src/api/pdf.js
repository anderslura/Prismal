...
export async function lastNedPDF(skjema, isPro = true) {
  ...
  ;(skjema.bom || []).forEach(b => {
    ...
    if (ant > 0 && pris > 0) rader.push(['Bom', String(ant), kr(pris), kr(ant*pris)])
  })
  ;(skjema.parkering || []).forEach(p => {
    const ant = parseFloat(p.antall)||0, pris = parseFloat(p.pris)||0
    if (ant > 0 && pris > 0) rader.push(['Parkering', String(ant), kr(pris), kr(ant*pris)])
  })

...
exports.handler = async (event) => {
  ...
  const resendPayload = {
    from: fraAdr,
    to: [tilEpost],
    reply_to: brukerEpost || skjema.firmaEpost || undefined,
  ...
  try {
    ...
    if (!res.ok) throw new Error(resendData?.message || `Resend HTTP ${res.status}`)
  } catch (err) {
    console.error('Resend feil:', err)
    return {
      statusCode: 500,
      body: JSON.stringify({ error: 'E-post kunne ikke sendes', detalj: err.message }),
    }
  }

  // Lagre historikk i Supabase (ikke-blokkerende)
  if (process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY) {
    try {
      ...
      console.error('Supabase-lagring feilet:', e)
    }
  }
